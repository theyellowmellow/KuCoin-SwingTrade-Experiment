...
apiKey = str(config['TOREPLACEWITHAPI'])
apiSecret = str(config['TOREPLACEWITHAPIESECRET'])
trade = config['ETH']
currency = config['UTK']
sellValuePercent = config.get('4', 4)
buyValuePercent = config.get('4', 4)
volumePercent = config.get('4', 4)
buyDifference = config.get('0', 0)
extCoinBalance = config.get('0', 0)
checkInterval = config.get('30', 30)
initialSellPrice = config.get('0', 0)
tradeAmount = config.get('0', 0)
